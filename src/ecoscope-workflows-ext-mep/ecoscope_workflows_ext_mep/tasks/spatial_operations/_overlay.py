...
def overlay_gdf(
    ...
    ------
    ValueError
        If `left` and `right` have mismatched CRS. geopandas would only warn
        and overlay the raw coordinates, so we fail loudly instead.
    """
    if left.crs != right.crs:
        raise ValueError(
            f"CRS mismatch: left={left.crs}, right={right.crs}. "
            "Reproject one frame to match the other before overlaying."
        )
