...
from typing import Union, Annotated,cast,List
...
#     print(f"Hull area (m²): {hull_area_m2}")
#     mcp_km2 = round(hull_area_m2 / 1_000_000.0, 1)
    
#     print(f"MCP area (km²): {mcp_km2}")

#     # Calculate ETD area
#     etd_km2 = 0.0
#     if etd_df is not None and not etd_df.empty:
#         etd_df = etd_df.to_crs(epsg=32737) 
...
def compute_subject_stats(
    ...
    # Calculate ETD area - WITH DEBUG PRINTS
    print(f"\n{'='*60}")
    print("ETD CALCULATION DEBUG")
    print(f"{'='*60}")
    
    etd_km2 = 0.0
    
    if etd_df is None:
        print("⚠️  ETD DataFrame is None")
    elif etd_df.empty:
        print("⚠️  ETD DataFrame is empty")
    else:
        print(f"✓ ETD DataFrame provided")
        print(f"  - Shape: {etd_df.shape}")
        print(f"  - Columns: {etd_df.columns.tolist()}")
        print(f"  - CRS: {etd_df.crs}")
        
        # etd_km2 = round(float(etd_df[etd_df["percentile"]== 99.9]["area"].sum()),1)
